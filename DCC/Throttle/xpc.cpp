    ...
    switch (xpc_state)
    {
        ...
        case XPC_WAIT_FOR_CALL:
            if (XP_rx_ready())
            {
                ...
                {
                    rx_message[0] = (unsigned char) (tmp & 0xFF);
                }
                ...
                if ((rx_message[0] & 0x1F) == XPNET_BROADCAST_ADDRESS)
                {
                    xpc_state = XPC_WAIT_FOR_MESSAGE; // deze state gaat de call byte overschrijven in rx_message[0]
                }
                // filteren op eigen address
                else if ((rx_message[0] & 0x1F) == xpc_MyAddress)
                {
                    if ((rx_message[0] & CALLBYTE_TYPE_FILTER) == CALL_ID)
                    {
                        if (xpc_Busy)
                        {
                    ...
                    else if ((rx_message[0] & CALLBYTE_TYPE_FILTER) == ACK_ID)
                    {
                    ...
                    else  if ((rx_message[0] & CALLBYTE_TYPE_FILTER) == MESSAGE_ID)
                    {
                ...
                if (XP_rx_ready())
                {
                    ...
                    if (rx_index == rx_size)
                    {
                        ...
                        if (my_check == 0)
                        {
                            ...
                            // else
                            xpc_parser();
                            xpc_state = XPC_WAIT_FOR_CALL; // xpc_Busy niet wijzigen hier! (dit kan een broadcast zijn terwijl de caller ondertussen een xpc request heeft gequeued )
                        }
                        else
                        {
