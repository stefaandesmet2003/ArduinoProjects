...
#include "status.h" // de status van de client die door xpnet rx msg wordt geupdate (bv fast clock, track status, feedback status)
#include "xpc.h"
...
static void xpc_SendMessage(uint8_t *msg)
{
  ...
  switch(rx_message[0] >> 4) {
    ...
    case 0x06 :
      ...
      else if (rx_message[1] == 0x10) // register mode programming results (eventueel implementeren voor pcintf module)
      {
        ...
        if (xpc_EventNotify) xpc_EventNotify (XPEVENT_PROG_SHORT);
      }
