...
unsigned int X_RxBuffer[X_RxBuffer_Size]; //sds client tx 8bit, rx 9bit

#define X_TxBuffer_Size  32
//unsigned int X_TxBuffer[X_TxBuffer_Size];       // this is int, we have 9 bits
unsigned char X_TxBuffer[X_TxBuffer_Size]; //sds client tx 8bit, rx 9bit
...
void XP_flush_rx(void)                 // Flush Receive-Buffer
  {
    ...
    // Rucksetzen von Receive Complete-Flags
...
ISR(USART_UDRE_vect) 
{
    // sds : opgelet ! deze code is anders dan rs485 master!!!
...
// keine uberlaufsicherung, da ja wir Master sind.
// sds : opgelet deze code is anders dan RS485 master!! je leest de 9de bit voor call-byte detectie!!
// dus X_RxBuffer zijn 16bits!!
ISR(USART_RX_vect)
{
    if (UCSR0A & (1<< FE0))
	{ // Frame Error
    ...
    else
    {
        ...
        //sds ; lees bit8
        X_RxBuffer[X_rx_write_ptr] = 0;
        if (UCSR0B & (1<< RXB80)) // bit 8 is 1
            X_RxBuffer[X_rx_write_ptr] = 0x100;
       	X_RxBuffer[X_rx_write_ptr] = X_RxBuffer[X_rx_write_ptr] | UDR0;

        X_rx_write_ptr++;
        if (X_rx_write_ptr == X_RxBuffer_Size) X_rx_write_ptr=0;
        X_rx_fill++;          // no check for full, we just do an overrun!
        
        //sds : overrun indication for debug
        if (X_rx_fill > X_RxBuffer_Size)
...
// Sendepuffer kann im Simulator mit einen Databreakpoint auf UDR uberwacht werden.
