...
// TODO : if saved state, herstel deze state uit de CV's
  ...
  for (int i = 0; i < 4; i++)
  {
    Serial.print("pulseDuration output ");
    Serial.print(i);
    Serial.print(" : ");
    Serial.print(pulseDuration[i] * 50);
    Serial.println("ms");
  }
#endif
} // turnout_Init
...
void turnout_Handler(uint16_t decoderAddress, uint8_t outputId, bool activate)
{
  ...
  if ((decoderAddress != getDecoderAddress()) && (decoderAddress != 511))
  { // 511 = broadcast
    return;
  }
  ...
  {
    for (int i = 0; i < 8; i++)
      digitalWrite(decoderOutputs[i], LOW);               // all outputs off
  ...
  { // 'off' command
    ...
    if (pulseDuration[outputId >> 1] == 0)
    { // no auto-off on the output, so switch off now
...
void output_Handler(uint16_t decoderAddress, uint8_t outputId, bool activate)
{
  ...
  if (decoderAddress == getDecoderAddress() + 1)
    physicalOutput = +4;
