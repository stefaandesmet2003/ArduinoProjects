...
#if (BIDI_TEST == 1)
#warning BIDI_TEST: internal code only, not for release
static void build_cs_unique(t_message *new_message)
  {
    // message 11111111 11111111 MANID UID3 UID2 UID1 UID0 XOR
    new_message->repeat = 1;
    new_message->type = is_stop;
    new_message->size = 7;
    new_message->dcc[0] = 0xFF;
    new_message->dcc[1] = 0xFD; // 0xFC
    new_message->dcc[2] = 13;       // DIY
    new_message->dcc[3] = 01;       // Produkt ID
    new_message->dcc[4] = 02;       // Session
    new_message->dcc[5] = 03;       // Serial
    new_message->dcc[6] = 04;       // Serial
    #if (MAX_DCC_SIZE < 7)
        #error: MAX_DCC_SIZE too small
    #endif
  }
        ...
        if (locobuffer[i].address == 0)
          {
            ...
            #if (BIDI_SUPPORT >= 2)
                locobuffer[lb_index].did = 0xFFFF;         // disallocate here
                locobuffer[lb_index].bidi_new = 0;
                locobuffer[lb_index].bidi_valid = 0;
            #endif
            locobuffer[lb_index].address = addr;
            locobuffer[lb_index].refresh = 0;
            locobuffer[lb_index].format = get_loco_format(addr);
...
#if (BIDI_SUPPORT >= 2)
void set_location_locobuffer(unsigned int addr, unsigned int did)
  {
    uint8_t i;
    organizer_state.bidi_new = 1;         // set flag for parser

    for (i=0; i<SIZE_LOCOBUFFER; i++)
      {
        if (locobuffer[i].address == (addr & 0x3FFF))
          {
            locobuffer[i].did = did;         // allocate here
            locobuffer[i].bidi_valid = 1;
            locobuffer[i].bidi_new = 1;
            if (addr & (1<<15)) locobuffer[i].bidi_dir = 1; 
            else locobuffer[i].bidi_dir = 0;
            return;
          }
      }
    // not found - create a new entry
    get_entry(255, (addr & 0x3FFF));    // slot = 255 - void
    locobuffer[lb_index].did = did;         // allocate here
    locobuffer[lb_index].bidi_valid = 1;
    locobuffer[lb_index].bidi_new = 1;
    if (addr & (1<<15)) locobuffer[lb_index].bidi_dir = 1; 
    else locobuffer[lb_index].bidi_dir = 0;
    return;
  } 
...
void clear_location_locobuffer(unsigned int did)               // remove assignment of a detector ID from loco
  {
    ...
    for (i=0; i<SIZE_LOCOBUFFER; i++)
      {
        if (locobuffer[i].did == did)         // disallocate here
          {
            locobuffer[i].bidi_valid = 0;
            locobuffer[i].bidi_new = 1;
          }
      }
  }
...
bool do_accessory(unsigned char slot, unsigned int addr, unsigned char output, unsigned char activate)      // turnout
  {
    ...
    #if (BIDI_TEST == 1)
        if (addr > 100) // DMX_MACRO_ADDR)
          {
            build_cs_unique(locobuff_mes_ptr);
            retval = put_in_queue_low(locobuff_mes_ptr);
            return(retval);
          }
