            ...
            if (locobuffer[i].owner_changed  && !locobuffer[i].owned_by_pc)
              {
                ...
                send_lok_stolen( locobuffer[i].slot, locobuffer[i].address);
              }
          }    
        organizer_state.lok_stolen_by_handheld = 0;      // thats all
    #endif
  }

#if (XPRESSNET_ENABLED == 1)
void send_manual_turnouts(void)
  {
    unsigned char no_trnt_events;
    t_data16 trnt_addr;

    no_trnt_events = get_number_of_manual_turnout_ops();                // get amount
    if (no_trnt_events == 0) organizer_state.turnout_by_handheld = 0;   // clear flag

    while (no_trnt_events)
      {
        trnt_addr.as_uint16 = recall_manual_turnout();                  // returns add + pos.
        trnt_addr.as_uint8[1] &= 0x7f;

        pcm_build[0] = 0x42;
        create_xpressnet_schaltinfo(trnt_addr.as_uint16, &pcm_build[1]);
        pc_send_lenz(pars_pcm = pcm_build);       
        no_trnt_events--;
      }
    organizer_state.turnout_by_handheld = 0;
  }
#endif
           ...
           switch(pcc[1])
             {
               ...
               case 0x28: // SO read (special command opendcc, no test for addr range)
                      {
                        unsigned char so_ack[5];
                        t_data16 xaddr;
                        xaddr.as_uint8[1] = pcc[2];
                        xaddr.as_uint8[0] = pcc[3];
                        if (xaddr.as_uint16 < 1024)
                          {
                            so_ack[0] = 0x24;
                            so_ack[1] = 0x28;
                            so_ack[2] = pcc[2];
                            so_ack[3] = pcc[3];
                            so_ack[4] = eeprom_read_byte((uint8_t *) xaddr.as_uint16);
                            pc_send_lenz(pars_pcm = so_ack);
                          }
                        else
                          {
                            pc_send_lenz(pars_pcm = pcm_unknown);
                          }
                        return;
                      }
                    break;
               case 0x29: // SO write
                      {
                        ...
                        if (xaddr.as_uint16 < 1024)
                          {
                            eeprom_write_byte((uint8_t *) xaddr.as_uint16, pcc[4]);
                            so_ack[0] = 0x24;
                            so_ack[1] = 0x28;
                            so_ack[2] = pcc[2];
                            so_ack[3] = pcc[3];
...
#if (XPRESSNET_ENABLED == 1)                    
  		                create_xpressnet_schaltinfo(addr, &pcm_build[1]);
#endif
  		                pc_send_lenz(pars_pcm = pcm_build);           
    		            }
    		            else
...
//SDS added #if - deze functie is enkel gedefinieerd met XPRESSNET_ENABLED = 1
#if (XPRESSNET_ENABLED == 1)                    
  		                create_xpressnet_feedback(addr, &pcm_build[1]);
#endif
  		                pc_send_lenz(pars_pcm = pcm_build);
    		            }
    		            return;
...
//SDS added #if
#if (S88_ENABLED == 1)
    if ((s88_event.hardware_change) || (s88_event.turnout_change))
      {
        make_s88_lenz_report();                         // report any Feedback-Change
      }
#endif // (S88_ENABLED == 1)    
