...
bool ui_doTurnoutMenu (uint8_t key)
{
    ...
    if (ui_State == UISTATE_TURNOUT_PAGE1)
    {
        ...
        {
            app_ToggleAccessory (ui_Page << 1); // wissel 1 = turnoutAddr 0
        }
        else if (keyCode == KEY_KEY3)
        {
            app_ToggleAccessory ((ui_Page << 1)+1); // wissel 2 = turnoutAddr 1
        }
        else if (keyCode == KEY_KEY4)
        {
            ui_Page++; // door de 255 pagina's scrollen, daarmee hebben we 512 wissels
        }
        else keyHandled = false;
    }
    return (keyHandled);
...
void ui_Update ()
{
    if ((millis() - updateLastMillis) > BACKLIGHTOFF_DELAY)
    {
        lcd.setBacklight(0);
    }
    //if ( (!ui_Redraw) && ((millis() - updateLastMillis) < REFRESH_DELAY)) //sds da werkt niet azo!
...
static bool handleSpeedKeys (key_t key)
{
    ...
    if (keyHandled)
    {
        app_SetSpeed(ui_CurLoc,ui_CurSpeed); 
    }
    return (keyHandled);
} // handleSpeedKeys
