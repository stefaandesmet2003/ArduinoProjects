 ...
 */
int8_t Timer_pulse(uint8_t pin, unsigned long period, uint8_t startingValue)
{
	return Timer_oscillate(pin, period, startingValue, 1); // once
}
 ...
 */
int8_t Timer_pulseImmediate(uint8_t pin, unsigned long period, uint8_t pulseValue)
{
	int8_t id = Timer_oscillate(pin, period, pulseValue, 1);
...
int8_t Timer_every(unsigned long period, void (*callback)(), int repeatCount)
{
	int8_t i = findFreeEventIndex();
	if (i == -1) return -1;
...
void Timer_update1 (unsigned long now)
{
	for (int8_t i = 0; i < MAX_NUMBER_OF_EVENTS; i++)
	{
		if (_events[i].eventType != EVENT_NONE)
